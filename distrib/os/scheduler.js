...
(function (TSOS) {
    class Scheduler {
        ...
        scheduleAfterQuantumExpired() {
            ...
            _KernelInterruptQueue.enqueue(new TSOS.Interrupt(DISPATCHER_SAVE_PROCESS, oldPCB.PID)); // save state of old PCB
            _KernelInterruptQueue.enqueue(new TSOS.Interrupt(DISPATCHER_MOVE_PROCESS, [0])); // the param does not matter for this call. Deals with the context switch to the new PCB
            ...
            _KernelInterruptQueue.enqueue(new TSOS.Interrupt(DISPATCHER_RUN_HEAD, newPCB.PID)); // run the new PCB which is either the one after the old one, or if there was just one process than just run the old one
        }
