    ...
    export class Shell {
        ...
        public init() {
            ...
            sc = new ShellCommand(this.writeFile,
                "write",
                "<filename> \"data\"- write the data inside the quotes (but not the quotes themselves) to 'filename'");
            this.commandList[this.commandList.length] = sc;
            ...
            // copy  
            sc = new ShellCommand(this.copyFile,
                "copy",
                "<existing filename> <new filename> - copies the contents from an existing file into a new file");
            this.commandList[this.commandList.length] = sc;

             // rename  
             sc = new ShellCommand(this.renameFile,
                "rename",
        ...
        public shellLoad() {
            ...
            if(isSwapSpaceAvailable) { // called explictly because an else would not work because this function makes sure the disk is formatted.
                ...
                let contentForSwapFile: string = program.map((num) => num.toString(16).toUpperCase().padStart(2, '0')).join(''); // turn each of these into the proper hex characters for the swapfile. Chat GPT 12/2/24 helped me deal with the 0's not working correctly
